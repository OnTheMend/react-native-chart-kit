...
class BarChart extends AbstractChart {
  ...
  renderBars = config => {
    ...
    // IT"S MAXIMUM VALUE FOR BARS OF CHART
    const configData = [100];
    const baseHeight = this.calcBaseHeight(configData, height)
    return data.map((x, i) => {
      const barHeight = this.calcHeight(x, configData, height)
      const barWidth = 10
      ...
      let firstStConColor = exCurBarData && exCurBarData[i] && exCurBarData[i].length > 0 ? true : false;
      let secondStConColor = exCurBarData && exCurBarData[i] && exCurBarData[i].length > 0 ? true : false;
      let thirdStConColor = exCurBarData && exCurBarData[i] && exCurBarData[i].length > 0 ? true : false && (exCurBarData[i][2] <= 0 && exCurBarData[i][1] <= 0) ? true : false;
      return (
        <G>
          <Defs>
